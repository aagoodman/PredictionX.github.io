function init() {
    ...
    d3.json("data/data.json", function(error, realizations) {
        ...
        var i, j, disks, games, all_points = [], csv_data = [], gamenumber = 0, run_all = false;
...
// 60 and 80
        ...
        var handleCollision = function (object, linearVelocity, angularVelocity) {
            ...
            if (current_disk >= disks) {
                ...
                if (current_game < games) {
                    ...
                    csv_data = csv_data.concat(all_points);
                    if (run_all){
                        runNextCell();
                    }
                }
        ...
        function Histogram() {
            ...
            this.margin = {bottom:20.525, left:0, right:0, top: 15.39375};
            //{top: 15 * window.innerHeight/800, right: 0, bottom: 20 * window.innerHeight/800, left: 0};
            this.width = 369.45;
            //360 * window.innerHeight/800;
            this.height = 244.2475;
            //238 * window.innerHeight/800;
...
//                console.log(this.height);
        ...
        function Instructions() {
            ...
            var instructions = ["Slide-the-puck",
                ...
                "Click on the link to learn more about uncertainty."]
...
//                    }
                    ...
                    if (word == 'link'){
                        text_class = 'text-website';
                        word = "<a href ='https://www.predictionx.org/introduction-to-uncertanity' id='"+text_class+"'>"+word+"</a>";
                    }
                    line.push(word);
                    if (idx == 0){
...
//
            ...
            d3.select('#text-download').on("click", function(d){
                //very crudely just updating the game number by updating the gae
//                console.log(csv_data);
                for (i = 0; i< csv_data.length; i++){
                        if (csv_data[i].x == -32){
                            gamenumber++;
                            csv_data[i].z = gamenumber;
                    }
                    else {
                        csv_data[i].z = gamenumber;
                    }
...
(function (root, m) {
  ...
  return function(arr, opts) {
    ...
    opts = opts || {};
    opts.filename = opts.filename || "puck-simulation.csv";
...
//    console.log(header);
      var header = "position on board, numbers of times the position was hit, game number" + opts.eol;
      ...
      setTimeout(function() {
        ...
        function Plot() {
            ...
            this.div = d3.select("#viewport");
                ...
                //innerheight
                .style("top", innerHeight/2 - this.height/2 );
        ...
        function fitGaussian(data){
            ...
            //This code was from fernando berrera
            ...
            //new code from Lily Nguyen
            var s = 0; //sum
            var xval = []; //array for position where the puck hit the board
            var numbars = []; //array to keep track of position and number of times puck hit that spot
            var bins = 0; // number of bars on the graph
            var p = 0; //numerator for standard deviation
            var mean = 0;
            var stdv = 0;
            var l = 0; //number of data points
            var ninecount = 0; // how many time a puck hit a position nine times
            var mode = 0;
            //maxfreq is the highest number of times a puck hits a position in one game
            var maxfreq = 0;
            for (i = 0; i < data.length; i++){
                if (data[i].y != 0){
                    l = l + 1;
